...
def get_data():
    ...
    print(df.columns)
    df.drop(columns=[' article', ' code_alea', ' title_article', ' estimated cost', ' fips', ' subcontinent', ' continent'], inplace=True)
    df['database'] = 'BD Catnat_exemple'
    df.drop(columns=[' other_countries_concerned', ' code_origine_alea', ' localization_fine', ' code_peril', ' code_under_peril', ' origin_alea'], inplace=True)
